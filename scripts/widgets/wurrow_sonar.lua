...
local Wurrow_Sonar = Class(Widget, function(self, owner)
    self.owner = owner
    Widget._ctor(self, "Wurrow_Sonar")
...
function Wurrow_Sonar:StartSonar()
...
function Wurrow_Sonar:SetSonar(off)
...
function Wurrow_Sonar:StopSonar()
    if self.active then
...
function Wurrow_Sonar:UpdateAlpha(dt)

    if self.time > 0 then
        self.time = math.max(0, self.time - dt)
    else
        if self.currentstate == "out" then
			if self.owner:HasTag("wurrowindark") and self.owner:HasTag("burrowed") then
				TheFocalPoint.SoundEmitter:PlaySound("UCSounds/vets_heartbeat/heartbeat_single")
				
				local ring = SpawnPrefab("wurrow_heartbeat_ringfx")
				...
				self.owner:DoTaskInTime(0.1, function()
						if self.active then
							local ring2 = SpawnPrefab("wurrow_heartbeat_ringfx")
            ...
            self.alphagoal = 0
            self.time = self.transitiontimeIN
        elseif self.currentstate == "in" and self.owner:HasTag("wurrowindark") then
            self.currentstate = "out"
            self.alphagoal = 1
...
function Wurrow_Sonar:OnUpdate(dt)
    if self.owner:HasTag("wurrow") and not TheNet:IsServerPaused() then
		self:UpdateAlpha(dt)
